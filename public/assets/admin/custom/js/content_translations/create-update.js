$(function () {
    let selected_group = $(`[data-role="selected-group"]`).val()?.trim(),
        selected_lang_id = $(`[data-role="selected-lang-id"]`).val()?.trim();

    $(`[data-role="group"], [data-role="lang_id"]`).select2({
        width: "100%",
    });

    if (selected_group) {
        $('[data-role="group"]').val(selected_group).trigger("change");
    }
    ...
    $(document).on("submit", `[data-role="form"]`, function (e) {
        ...
        let parent = $(this).closest(".row"),
            requiredFields = ["key", "value", "group", "lang_id"],
    ...
    $(document).on("input", `[data-role="key"]`, function (e) {
