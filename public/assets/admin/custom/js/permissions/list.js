$(function () {
    ...
    const trComponent = (d, i) => {
        return  `<tr data-id="${d.id}">
                    ...
                    <td width="100px" class="text-end">
                        <a href="${permissions_edit_route.replace('permission_id', d.id)}">
    ...
    $(document).on("click", `[data-role="btn-delete"]`, function () {
        ...
        Swal.fire({
            title: ` <b class="text-danger">${name}</b> icazəsini silmək istədiyinizə əminsiniz?`,
            showDenyButton: true,
            ...
            if (result.isConfirmed) {
                $.post({
                    url: permissions_delete_route.replace('permission_id', id),
                    data: {
                        id,
                        _method: "DELETE"
                    },
    ...
    $(document).on("change", `[data-role="change-status"]`, function () {
        ...
        Swal.fire({
            title: ` <b class="text-danger">${name}</b> icazəsinin statusunu dəyişmək istədiyinizə əminsiniz?`,
            showDenyButton: true,
            ...
            if (result.isConfirmed) {
                $.post({
                    url: permissions_change_status_route.replace('permission_id', id),
                    data,
                    success: function (d) {
                        if ([201, 202].includes(d.code)) {
